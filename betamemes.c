    ...
    while(running){
        ...
        if(main_menu == true){//if the game state is in the main menu
            ...
            if(game_over == true){//if you lose
                ...
                source_rect.x += 10;
                if(source_rect.x >= 100){
                    source_rect.x = 0;
                    source_rect.y += 26;
                }
                if(source_rect.y >= 156) {
                    source_rect.x = 0;
                    source_rect.y = 0;
                }
                ...
                if(mode == 0) {//mode 0 so objects
                    ...
                    SDL_Rect rect4 = {ob3x, oby, 30, 78};
                    SDL_SetRenderDrawBlendMode(renderer, SDL_BLENDMODE_BLEND);
                    SDL_RenderCopy(renderer, fireRedTexture, &source_rect, &rect4);
                    ...
                    SDL_Rect rect7 = {ob6x, ob2y, 30, 78};
                    SDL_SetRenderDrawBlendMode(renderer, SDL_BLENDMODE_BLEND);
                    SDL_RenderCopy(renderer, fireRedTexture, &source_rect, &rect7);
